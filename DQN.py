...
'''
def value_next_states(previous_model, list_of_next_states, N_samples):
    t0 = time.time()
    ...
    next_state_predicted_action_values = previous_model.predict([next_states, next_state_actions_to_value], batch_size=256)
    next_state_predicted_values = np.max(next_state_predicted_action_values, axis=1)
    return next_state_predicted_values


def return_training_batch(previous_experiences, previous_model, N_samples=800):
    tA = time.time()
    #previous_model = load_model(previous_model_filename)
    #print_in_color("SIZE={0}".format(os.path.getsize(previous_model_filename)), "yellow")

    list_of_next_states = []
    list_of_previous_experiences = []
    tB = time.time()
    for i in range(0, N_samples):
        experience = (state, action, reward, bool_game_complete, next_state) = random.choice(previous_experiences)
    ...
    for i in range(0, N_samples):
        ...
        if bool_game_complete == True:
            ...
            gamma = 0.99
            expected_reward = reward + gamma * next_state_values[i]
    ...
    tF = time.time()
    print_in_color("__________________________________", "cyan")
    print_in_color("T1={0}".format(tB-tA), "cyan")
    print_in_color("T2={0}".format(tC-tB), "cyan")
    print_in_color("T3={0}".format(tD-tC), "cyan")
    print_in_color("T4={0}".format(tE-tD), "cyan")
    print_in_color("T5={0}".format(tF-tE), "cyan")
    print_in_color("__________________________________", "cyan")
    previous_model = None
